...
def _call_openai(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    last_err = None
    for attempt in range(6):  # up to 5 retries with exponential backoff
        try:
            ...
            # 402/429 = rate limit / credit exhaustion -> back off and retry
            if status in (402, 429, 500, 502, 503, 504):
                last_err = e
                wait = min(2 ** attempt * 5, 60)  # 5s, 10s, 20s, 40s, 60s
                print(f"  [judge] API {status}, retrying in {wait}s (attempt {attempt + 1}/6)...")
                time.sleep(wait)
                continue
            # Non-retryable error (e.g. 400 content filter) -> fail closed safely
            judge_ms = (time.perf_counter() - t0) * 1000
            return JudgeVerdict(verdict=False, reason=f"[judge blocked/error: {e}]", judge_ms=judge_ms, provider="openai", raw=str(e))
        except Exception as e:
            judge_ms = (time.perf_counter() - t0) * 1000
            return JudgeVerdict(verdict=False, reason=f"[judge error: {e}]", judge_ms=judge_ms, provider="openai", raw=str(e))
    judge_ms = (time.perf_counter() - t0) * 1000
    return JudgeVerdict(verdict=False, reason=f"[judge failed after retries: {last_err}]", judge_ms=judge_ms, provider="openai", raw=str(last_err))
...
def _call_anthropic(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    try:
        ...
        # Handle content-blocked, rate limits, or bad requests gracefully (fail closed)
        judge_ms = (time.perf_counter() - t0) * 1000
        return JudgeVerdict(verdict=False, reason=f"[judge blocked/error: {e}]", judge_ms=judge_ms, provider="anthropic", raw=str(e))
    judge_ms = (time.perf_counter() - t0) * 1000
